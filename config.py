...
DEFAULT_AGGREGATE_PROMPT = (
    ...
    "要求：\n"
    "1) title 必须使用简体中文，只写用于卡片页眉的短主题标题，控制在 16 个中文字符左右；如果主题可以写成“主标题：副标题”，只输出冒号前的主标题，不要包含冒号和冒号后的内容；\n"
    "2) sections[].title 和 sections[].summary 必须使用简体中文，不要保留英文原文；\n"
    "3) sections 最多 {max_items} 段，每段概括一个主题或事件；\n"
    "4) 多个来源报道同一事件时合并成一段，并在 item_indices 中列出来源条目序号；\n"
    "5) image_index 选择最适合该段配图的条目序号，没有配图可省略或填 0；\n"
    "6) 不要输出 Markdown 代码块。\n\n"
    "任务 ID：{job_id}\n"
    "条目：\n{items}"
)
