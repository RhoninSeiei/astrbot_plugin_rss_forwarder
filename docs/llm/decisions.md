...
多目标轮询任务会在调度层追加目标维度，实际确认键为 `job:{job_id}:origin:{origin_sha256}:{item_id}`。升级前已经存在的全局内容去重键仍会被读取，用于避免旧内容在部署后重新进入推送。
...
同一个轮询任务内的语义判定和内容处理只执行一次，发送阶段按目标生成消息内容。某个目标发送成功后会记录该目标的内容确认键，其他目标失败时保留为待发送状态，后续轮询只补发未确认目标。

### 简洁模式图片

`jobs[].compact_mode_send_images` 默认关闭，维持“只推送标题”的原行为。开启后，简洁模式仍只保留标题文本，但会附带条目图片；文本模式把图片放入同一消息链，图片图卡模式先发送标题图卡，再发送原文图片。
