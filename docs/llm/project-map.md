...
- 启动与停止轮询任务。
- 执行 job 抓取、解析、归档、分发；支持 interval 与 cron 两种触发方式。
- 执行日报抓取与日报发送。
...
任务组级语义重复判定。负责读取当前 `job` 的近期代表新闻，调用 AstrBot LLM Provider 判断新条目是否属于同一事实事件，并在模型异常时返回允许推送。
...
5. 如当前 `job` 开启语义重复判定，`semantic_dedup.py` 比较当前条目与任务组近期代表新闻。
6. 如当前 `job` 开启聚合推送，`pipeline.py` 生成动态标题和分段摘要，`dispatcher.py` 发送文本或本地图卡。
6. `pipeline.py` 做翻译、摘要与正文清洗。
7. `dispatcher.py` 做发送前查重与消息发送。
8. `storage.py` 写入去重键、语义候选记录与 feed 状态。
...
- 运行时注册名与本地状态命名空间：`astrbot_plugin_rss_forwarder`
- 旧本地状态命名空间：`astrbot_rss`

首次使用新命名空间时会从旧状态目录复制去重记录，旧目录仅作为兼容来源保留。
