...
`verify_ssl` は `feeds[]` の各ソース設定にあり、`RSS/Atom ソース` と `Twitter/Nitter ソース` の両テンプレートで利用できます。通常は有効のまま使用し、ソースサーバーの身元を検証して通信中のコンテンツ偽装を防いでください。この設定が適用されるのは RSS ドキュメント、Nitter タイムライン、Nitter ツイート詳細のリクエストだけです。記事本文、画像、動画、その他のメディア取得では常に厳格な証明書検証を使用します。

AstrBot v4.24.1 は Plugin Pages をサポートしています。このバージョンでは、入口がページのディレクトリ名 `source-diagnostics` として表示される場合があります。プラグイン i18n に対応した新しい AstrBot では、ローカライズされたタイトル `ソース診断` が表示されます。`プラグイン詳細` から該当する入口を選択してページを開きます。v4.24.1 より前のバージョンでも、ポーリング機能と `/rss probe` コマンドは引き続き利用できます。診断結果では次の 4 モードを使用します。

- `direct_strict`：既定のネットワークを使用し、厳格な証明書検証を行います。
- `proxy_strict`：ソースの `proxy_url` を使用し、厳格な証明書検証を行います。
- `direct_relaxed`：既定のネットワークで証明書検証を無効にし、失敗が証明書に起因するかを確認する場合だけ使用します。
- `proxy_relaxed`：ソースプロキシで証明書検証を無効にし、プロキシ経由の証明書問題を確認する場合だけ使用します。

`relaxed` モードの成功は、証明書検証を無効にするとアクセスできることを示し、証明書に関するセキュリティ上の警告を伴います。通常は厳格な検証を維持し、証明書チェーンの問題を確認してセキュリティ上の影響を受け入れた場合に限り、そのソースの `verify_ssl` を変更してください。ページに表示される推奨設定は自動保存されないため、ソース設定で手動適用します。

旧バージョンの AstrBot またはモバイル管理では、管理者の会話で `/rss probe <feed_id>` を実行して保存済みソースを検査できます。このコマンドは同じモード結果と推奨設定を表示し、ソース設定やポーリング状態は変更しません。
