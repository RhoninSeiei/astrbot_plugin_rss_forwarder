...
RSS 源可按需设置 `send_images`、`send_videos` 与 `max_new_items`。其中 `max_new_items=0` 表示不按单个源限制每轮解析数量，实际推送数量仍受轮询任务的 `batch_size` 限制。
...
如果某个轮询任务只需要标题提醒，可以开启 `compact_mode_enabled`。开启后该任务默认只推送标题，不显示来源、时间、摘要、网页链接和原文媒体。单个目标可通过 `targets[].compact_mode` 覆盖该默认值，例如群聊设为 `compact`，私聊设为 `normal`。需要在简洁模式下保留图片时，可开启 `compact_mode_send_images`。
...
- `direct_relaxed`：使用默认网络并关闭证书校验，仅用于确认失败是否与证书有关。
- `proxy_relaxed`：使用来源代理并关闭证书校验，仅用于确认代理访问下的证书问题。

`relaxed` 模式成功表示关闭证书校验后可以访问，同时存在证书安全隐患。正常使用时仍建议保留严格校验，仅在确认来源证书链异常并接受相关安全影响后，才为该来源调整 `verify_ssl`。页面给出的建议不会自动保存，需要在来源设置中手动修改。

旧版 AstrBot 或移动端管理场景可在管理员会话中使用 `/rss probe <feed_id>` 检查已保存来源。该命令显示各模式结果与建议，也不会修改来源设置或轮询状态。
...
- `jobs[]`
  ...
  - `compact_mode_send_images`（简洁模式下附带图片）
