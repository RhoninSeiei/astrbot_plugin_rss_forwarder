...
class _RequestStub:
    ...
    async def invoke(self, handler, body, username):
...
class SourceProbeApiRunTests(unittest.IsolatedAsyncioTestCase):
    async def _run(self, api, body, username="operator"):
        return await request_stub.invoke(api.run_probe, body, username)
    ...
    async def test_none_username_uses_one_fixed_string_lock_key(self):
        ...
        api = SourceProbeApi(_config(_rss_feed()), _Service(block))
        first = asyncio.create_task(
            self._run(api, {"feed_id": "rss-1"}, username=None)
        )
        await entered.wait()

        self.assertEqual(len(api._probe_locks), 1)
        self.assertTrue(all(isinstance(key, str) for key in api._probe_locks))
        duplicate = await self._run(
            api,
            {"feed_id": "rss-1"},
            username=None,
